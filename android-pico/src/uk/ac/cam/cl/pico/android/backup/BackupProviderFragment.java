...
import uk.ac.cam.cl.pico.android.util.PauseHandler;
import uk.ac.cam.cl.pico.android.util.ProgressDialogFragment;
import uk.ac.cam.cl.pico.backup.BackupFileException;
...
import uk.ac.cam.cl.pico.gson.EncBackupFileGson;
...
public abstract class BackupProviderFragment extends Fragment implements IBackupProvider {
    ...
    final static String ACTION_RESTORE_LATEST =
    		"uk.ac.cam.cl.pico.android.backup.restore_latest";
    final static String ACTION_RESTORE_USER_SELECTED =
    		"uk.ac.cam.cl.pico.android.backup.restore_user_selected";
    final static String ACTION_USER_SECRET_ENTERED =
    		"uk.ac.cam.cl.pico.android.backup.user_secret_entered";
     ...
     * @see http://http://stackoverflow.com/questions/8040280/how-to-handle-handler-messages-when-activity-fragment-is-paused
     */
     ...
     * @see http://http://www.androiddesignpatterns.com/2013/04/activitys-threads-memory-leaks.html
     * @author Graeme Jenkinson <gcj21@cl.cam.ac.uk>
     * */
		...
		public void run() {
			...
			try {
	            ...
	            try {
		            ...
		            final File dbFile = PicoApplication.getContext()
		            		.getDatabasePath(DbHelper.DATABASE_NAME);	            
		            if (!dbFile.exists()) {
	...
	protected void createBackupStarted() {
    	...
    	mBuilder.setContentTitle(applicationContext.getText(
          ...
          .setSmallIcon(R.drawable.ic_launcher_pico);
	...
	public EnumSet<RestoreOption> getRestoreOptions() {
		return EnumSet.allOf(RestoreOption.class);
	}
