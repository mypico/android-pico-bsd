...
import static uk.ac.cam.cl.pico.android.backup.IBackupProvider.BackupType.GOOGLEDRIVE;
...
public final class GoogleDriveBackupProviderFragment extends BackupProviderFragment {
     ...
     *
     */
    private final class RestoreBackupResultCallback implements ResultCallback<ContentsResult> {
        
        @Override
        public void onResult(final ContentsResult result) {
        	downloadBackupCompleted();
        	
            if (result.getStatus().isSuccess()) {                         
                try {
                    // Get the InputStream of the file to restore from Google Drive
                    final InputStream googleDriveIs = result.getContents().getInputStream();
	                    try {
	                	// Copy the file to restore from Google Drive to a temporary file prior
	                	// to decryption
	                    final File tempFile = File.createTempFile(DbHelper.DATABASE_NAME, null,
    ...
    public void onActivityResult(final int requestCode, final int resultCode,
        ...
        case PICK_FILE_REQUEST_CODE:
            if (resultCode == Activity.RESULT_OK) {
                ...
                backupFile.openContents(mGoogleApiClient, DriveFile.MODE_READ_ONLY,
        	...
        	.setResultCallback(new ResultCallback<ContentsResult>() {
                ...
                public void onResult(final ContentsResult result) {
                   ...
                   try {
                       ...
                       try {          
                    	   // Guava ByteStreams.copy() does not flush or close either stream
	                       copy(jsonEncIs, result.getContents().getOutputStream());
