...
import static uk.ac.cam.cl.pico.android.backup.IBackupProvider.BackupType.SDCARD;
import uk.ac.cam.cl.pico.android.core.PicoApplication;
import uk.ac.cam.cl.pico.android.db.DbHelper;
import uk.ac.cam.cl.pico.android.util.AsyncTaskResult;
import uk.ac.cam.cl.pico.backup.BackupKeyException;
import uk.ac.cam.cl.pico.backup.BackupFile;
...
public final class SdCardBackupProviderFragment extends BackupProviderFragment {
    ...
    private final class RestoreBackupTask
    	...
    	private final File tempFile = File.createTempFile(DbHelper.DATABASE_NAME, null,
        ...
        protected AsyncTaskResult<Boolean> doInBackground(final File... params) {
            ...
            try {
                ...
                // Created the Pico database if it doesn't already exist
                if(!dbFile.exists()) {
                    dbFile.getParentFile().mkdirs();
                    dbFile.createNewFile();
                } 
                
                // Restore the Pico database - copy(From, To)
                copy(backupFile, dbFile);
                return new AsyncTaskResult<Boolean>(true);
            } catch (IOException e) {               
                // Error creating Pico database file   
                LOGGER.error("Error restoring the Pico database file", e);      
                return new AsyncTaskResult<Boolean>(e);
            }
        }    
    ...
    protected void createBackup() {       
        ...
        final File dbFile = getActivity().getDatabasePath(DbHelper.DATABASE_NAME); 
    ...
    public void restoreLatestBackup() {
        ...
        final File dbFile = PicoApplication.getContext().getDatabasePath(DbHelper.DATABASE_NAME);   
        final File sdCardDir = new File(Environment.getExternalStorageDirectory().getPath());
        try {
	        new RestoreBackupTask().execute(dbFile, new File(sdCardDir, PICO_BACKUP_DIR));  
        } catch (IOException e) {
